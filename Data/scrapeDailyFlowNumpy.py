...
outDir = "all_flow"
startYear = 1972 # must be a leap year!
...
for j in range(len(links)):
    ...
    for value in rowData:
        numeric1 = r'\d+\.\d+'
        numeric2 = r'\d+'
        if re.search(numeric1, value) or re.search(numeric2, value):
            rowData[index] = float(rowData[index])
        else:
            rowData[index] = None

        index += 1

    outPath = os.path.join(outDir, catchment)
    np.save(outPath, rowData)
